...
drop policy if exists financial_ledger_finance_read on public.financial_ledger;
create policy financial_ledger_finance_read on public.financial_ledger for select
  using(public.is_member_of(organization_id) and (public.has_permission('invoices.manage') or public.has_permission('reports.finance')));
